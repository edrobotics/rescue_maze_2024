...
void Adafruit_TCS34725::write8(uint8_t reg, uint8_t value) {
  ...
  uint8_t realReg {buffer[0]};
  uint8_t myBuf[1] = {value};
  _i2cComm->writeReg(_addr, realReg, 1, myBuf);
...
uint8_t Adafruit_TCS34725::read8(uint8_t reg) {
  ...
  uint8_t realReg {buffer[0]};
  _i2cComm->readReg(_addr, realReg, 1, buffer);
...
uint16_t Adafruit_TCS34725::read16(uint8_t reg) {
  ...
  uint8_t realReg {buffer[0]};
  _i2cComm->readReg(_addr, realReg, 2, buffer);
 ...
 *          Gain
 */
Adafruit_TCS34725::Adafruit_TCS34725(uint8_t it, tcs34725Gain_t gain, i2cCommunicator* i2cComm) {
...
void Adafruit_TCS34725::clearInterrupt() {
  ...
  uint8_t realReg {buffer[0]};
  _i2cComm->writeReg(_addr, realReg, 0, buffer);
...
void Adafruit_TCS34725::test()
{
    ...
    while (true)
    {
        ...
        std::cout << "Colour Temp: " << colorTemp << " K - "
        << "Lux: " << lux << " - "
        << "R: " << r << " "
        << "G: " << g << " "
        << "B: " << b << " "
